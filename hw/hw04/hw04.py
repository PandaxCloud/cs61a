...
def replace_leaf(t, old, new):
    ...
    #   if label(t) == old:
    #     return tree(new)
    # to_be_modified = copy_tree(t)
    # for branch in branches(to_be_modified):
    #     if is_leaf(branch):
    #         if label(branch) == old:
    #             branch = tree(new)
    #     else:
    ...
    if is_leaf(t):
        if label(t) == old:
            return tree(new)
    new_branchs = [replace_leaf(b, old, new) for b in branches(t)]
...
def make_withdraw(balance, password):
    ...
    """
    incorrect_passwords = []
    def withdraw(amount, typed_in_password):
        nonlocal balance, incorrect_passwords
        if not len(incorrect_passwords) < 3:
            return "Your account is locked. Attempts: " + str(incorrect_passwords)
        elif typed_in_password == password:
            if amount > balance:
            ...
            incorrect_passwords += [typed_in_password] 
...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(response) == str:
        return response
    def joint_withdraw(amount, typed_in_password):     
        if typed_in_password in [old_password, new_password]:
            return withdraw(amount, old_password)
        else:
            return withdraw(amount, typed_in_password)
...
def quadratic(x, a, b, c):
    ...
    # extreme_value, left_value, right_value = map(f, (axis_of_symmetry, lower_bound(x), upper_bound(x)))
    extreme_value = a * axis_of_symmetry * axis_of_symmetry + b * axis_of_symmetry + c
    p1 = lower_bound(x)
    left_value = a * p1 * p1 + b * p1 + c
    p2 = upper_bound(x)
    right_value = a * p2 * p2 + b * p2 + c
    if lower_bound(x) <= axis_of_symmetry <= upper_bound(x):
