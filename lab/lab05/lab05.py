...
def add_trees(t1, t2):
    ...
    added_branches = [add_trees(b1, b2) for b1, b2 in zip(branches(t1), branches(t2))]
    if len(branches(t1)) == len(branches(t2)):
        return tree(label(t1) + label(t2), added_branches)
    if len(branches(t1)) < len(branches(t2)):
        unchanged_branches = branches(t2)[len(branches(t1)):]
        return tree(label(t1) + label(t2), added_branches + unchanged_branches)
    if len(branches(t1)) > len(branches(t2)):
        unchanged_branches = branches(t1)[len(branches(t2)):]
        return tree(label(t1) + label(t2), added_branches + unchanged_branches)
